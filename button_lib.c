...
static nrf_drv_gpiote_pin_t pin;
...
static void gpiote_scheduled_handler(void * p_event_data, uint16_t event_size)
{
    uint32_t err_code;
...
static void gpiote_event_handler(nrf_drv_gpiote_pin_t n_pin, nrf_gpiote_polarity_t n_action)
{
	pin = n_pin;
	uint32_t err_code = app_sched_event_put(NULL, 0, gpiote_scheduled_handler);
