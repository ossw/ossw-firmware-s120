...
static void button_handler(uint8_t p, uint8_t a) {
	pin_no = p;
	button_action = a;
	uint32_t err_code = app_sched_event_put(NULL, 0, button_press_event);
...
static void button_long_press_timeout_handler(void * p_context) {
	pin_no = (uint8_t)((uint32_t)p_context);
	uint32_t err_code = app_sched_event_put(NULL, 0, button_long_press_event);
	APP_ERROR_CHECK(err_code);
}
